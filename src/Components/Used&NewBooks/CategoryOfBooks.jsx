...
const CategoryOfBooks = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      const res = await axios.get(`${process.env.REACT_APP_URL}/Books/new-book`, {
  ...
  useEffect(() => {
    const fetchBooks2 = async () => {
      const res = await axios.get(`${process.env.REACT_APP_URL}/Books/used-book`, {
        headers: { Authorization: `Bearer ${token}` },
      });
        ...
        {displayBooks.map((book, idx) => (
          <Card   onClick={()=>navigate(`/single-book/${book._id}`)}className="slide" key={idx}>
            ...
            {book.image && book.image[0] && (
              <CardMedia
                component="img"
                image={`${process.env.REACT_APP_URL}/${book.image[0]}`}
                alt={book.bookname}
